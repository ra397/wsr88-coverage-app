...
def worker(args):
    qr, easting, northing, tower_ft, max_alt, elevation_angles = args
...
def calculate_coverage(easting, northing, tower_ft=None, max_alt=3000, elevation_angles=None):
    if tower_ft is None:
    ...
    print("Calculating coverage with: ", easting, northing, tower_ft, max_alt, elevation_angles)

    total_tm = time()
    with Pool(4) as pool:
        results = pool.map(worker, [(qr, easting, northing, tower_ft, max_alt, elevation_angles) for qr in QRTS])
...
if __name__ == "__main__":
    calculate_coverage(448049.26648561255, 2080241.2158965976, elevation_angles=[0.5, 19.5])